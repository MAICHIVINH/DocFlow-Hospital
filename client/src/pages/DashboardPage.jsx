        ...
        <div className="rounded-2xl bg-theme-secondary p-6 shadow-sm border border-theme">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <h3 className="mt-1 text-2xl font-bold text-theme-primary">{value}</h3>
                    {trend && (
...
const DashboardPage = () => {
    ...
    return (
        <div className="space-y-8">
            <div>
                <h2 className="text-2xl font-bold text-theme-primary transition-all">{t('greeting')}</h2>
                <p className="text-theme-secondary">{t('dashboard_subtext')}</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                {/* Chart */}
                <div className="rounded-2xl bg-theme-secondary p-6 shadow-sm border border-theme lg:col-span-1">
                ...
                <div className="rounded-2xl bg-theme-secondary p-6 shadow-sm border border-theme lg:col-span-2">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-bold text-theme-primary font-primary">{t('recent_docs')}</h3>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-theme text-left text-xs font-bold uppercase tracking-wider text-theme-secondary">
                                    ...
                                    <tr key={doc.id} className="group hover:bg-theme-primary transition-colors">
                                        <td className="py-4">
                                            <div className="flex items-center">
                                                <div className="mr-3 rounded-lg bg-theme-primary p-2 text-theme-secondary group-hover:bg-theme-secondary group-hover:shadow-sm">
                                                    <FileText className="h-4 w-4" />
                                                </div>
                                                <span className="font-medium text-theme-primary">{doc.title}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 text-sm text-theme-secondary">{doc.department?.name || t('unassigned')}</td>
                                        <td className="py-4">
                                        ...
                                        <td className="py-4 text-sm text-theme-secondary">{new Date(doc.created_at).toLocaleDateString(t('vi-VN') === 'vi-VN' ? 'vi-VN' : 'en-US')}</td>
                                        <td className="py-4 text-right">
                                            <button className="rounded-lg p-2 text-theme-secondary hover:bg-theme-secondary hover:text-hospital-600 hover:shadow-sm">
